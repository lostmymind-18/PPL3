...
class ASTGeneration(D96Visitor):
    ...
    def visitLiteral(self,ctx:D96Parser.LiteralContext):
        if ctx.LIT_INT():
            return IntLit(ctx.LIT_INT().getText())
        if ctx.LIT_FLOAT():
            return FloatLit(ctx.LIT_FLOAT().getText())
        if ctx.lit_boolean():
    ...
    def visitStatic_method(self,ctx:D96Parser.Static_methodContext):
        return MethodDecl(Static(),ctx.ID_STATIC().getText(),self.visit(ctx.list_para()),self.visit(ctx.stat_block()))

    def visitInstance_method(self,ctx:D96Parser.Instance_methodContext):
        return MethodDecl(Instance(),ctx.ID().getText(),self.visit(ctx.list_para()),self.visit(ctx.stat_block()))

    def visitConstructor_method(self,ctx:D96Parser.Constructor_methodContext):
        return MethodDecl(Instance(),ctx.CONSTRUCTOR().getText(),self.visit(ctx.list_para()),self.visit(ctx.stat_block()))
    
    def visitDestructor_method(self,ctx:D96Parser.Destructor_methodContext):
        return MethodDecl(Instance(),ctx.DESTRUCTOR().getText(),[],self.visit(ctx.stat_block()))
    ...
    def visitStat_var_con(self,ctx:D96Parser.Stat_var_conContext):
        ...
        if ctx.VAR():
            if ctx.stat_var_con1():
                ...
                for i in range(1,len(listId)):
                    ret+=VarDecl(listId[i][0],listId[0],listId[i][1])
        ...
        if ctx.VAL():
            if ctx.stat_var_con1():
                listId = self.visit(ctx.stat_var_con1())
                for i in range(1,len(listId)):
                    ret+=ConstDecl(listId[i][0],listId[0],listId[i][1])
    ...
    def visitStat_var_con1(self,ctx:D96Parser.Stat_var_con1Context):
        x = self.visit(ctx.list_id_ins())
        for i in x:
            i=[i,None]
        x=[self.visit(ctx.mytype())]+x
        return x
    ...
    def visitStat_foreach(self,ctx:D96Parser.Stat_foreachContext):
        return For(self.visit(ctx.mpid()),self.visit(ctx.expr()[0]),self.visit(ctx.expr[1]),self.visit(ctx.stat_block()),self.visit(ctx.by_expr()))
    ...
    def visitStat_method_sta(self,ctx:D96Parser.Stat_method_staContext):
        if ctx.list_expr():
            return Call(ctx.ID().getText(),ctx.ID_STATIC().getText(),self.visit(ctx.list_expr()))
        return Call(ctx.ID().getText(),ctx.ID_STATIC().getText(),[])
        
    def visitStat_method_ins(self,ctx:D96Parser.Stat_method_insContext):
        if ctx.list_expr():
            return Call(self.visit(ctx.expr8()),ctx.ID().getText(),self.visit(ctx.list_expr()))
        return Call(ctx.ID().getText(),ctx.ID_STATIC().getText(),[])
    ...
    def visitExpr2(self,ctx:D96Parser.Expr2Context):
        if ctx.getChildCount() > 1:
            return BinaryOp(self.visit(ctx.op_log2()),self.visit(ctx.expr2()),self.visit(expr3()))
    ...
    def visitExpr6(self,ctx:D96Parser.Expr6Context):
        if ctx.getChildCount() > 1:
            return UnaryOp(self.visit(ctx.MINUS().getText()),self.visit(ctx.expr6()))
        return self.visit(ctx.expr7())

    def visitExpr7(self,ctx:D96Parser.Expr7Context):
        if ctx.getChildCount() > 1:
            lst = self.visit(ctx.expr8())
            return ArrayCell(lst[0],lst[1:]+lst[-1])
        return self.visit(ctx.expr9()) 

    def visitExpr8(self,ctx:D96Parser.Expr8Context):
        if ctx.getChildCount() > 1:
            return self.visit(ctx.expr8())+[self.visit(ctx.expr())]
        return [self.visit(ctx.expr9())]

    def visitExpr9(self,ctx:D96Parser.Expr9Context):
        if ctx.getChildCount() == 3:
            return FieldAccess(self.visit(ctx.expr9()),ctx.ID().getText())
        if ctx.getChildCount() > 3:
            if ctx.list_expr():
                return CallExpr(self.visit(ctx.expr9()),ctx.ID().getText(),self.visit(ctx.list_expr()))
            return CallExpr(self.visit(ctx.expr9()),ctx.ID().getText(),[])
        if ctx.getChildCount() == 1:
            return self.visit(ctx.expr10())
    def visitExpr10(self,ctx:D96Parser.Expr10Context):
        if ctx.getChildCount()==3:
            return FieldAccess(ctx.ID().getText(),ctx.ID_STATIC().getText())
        if ctx.getChildCount()>3:
            if ctx.list_expr():
                return CallExpr(ctx.ID().getText(),ctx.ID_STATIC().getText(),self.visit(ctx.list_expr()))
            return CallExpr(ctx.ID().getText(),ctx.ID_STATIC().getText(),[])
        if ctx.getChildCount() == 1:
    ...
    def visitLit(self,ctx:D96Parser.LitContext):
        ...
        if ctx.lit_boolean:
            return BooleanLiteral(self.visit(ctx.lit_boolean()))
        if ctx.indexed_arr():
            return self.visit(ctx.indexed_arr())
        if ctx.lit_mul_arr():
